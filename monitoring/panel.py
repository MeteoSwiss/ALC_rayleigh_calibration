...
PANEL_CSS = r"""
 .dp-wrap { --dp-line:#dbe3ea; --dp-ink:#1a2530; --dp-dim:#66707a; --dp-ok:#2ca02c; --dp-bad:#b00020; }
 .dp-wrap, .dp-wrap * { box-sizing:border-box; }
 ...
 .dp-wrap .dp-rail { position:absolute; top:0; right:100%; margin-right:14px; width:232px; }
 ...
 @media (max-width:1779px) { .dp-wrap:not(.rail-hidden) .dp-rail {
   ...
   background:#f7f9fb; border:1px solid var(--dp-line); border-radius:12px; padding:8px;
 ...
 .dp-wrap button:hover:not(:disabled) { background:#eef4fb; border-color:#2a5a82; }
 .dp-wrap button:disabled { opacity:.4; cursor:default; }
 .dp-wrap .seg { display:inline-flex; border:1px solid #c3ceda; border-radius:7px; overflow:hidden; }
 .dp-wrap .seg button { border:0; border-radius:0; }
 .dp-wrap .seg button + button { border-left:1px solid #c3ceda; }
 .dp-wrap .seg button.on { background:#2a5a82; color:#fff; }
 .dp-wrap .datelbl { font-weight:600; font-size:15px; min-width:118px; text-align:center; }
 .dp-wrap .chip { display:inline-block; padding:2px 9px; border-radius:11px; font-size:12px;
         font-weight:600; border:1px solid transparent; }
 .dp-wrap .chip.ok { background:#e8f6ea; color:#1a6b28; border-color:#b6e0bd; }
 .dp-wrap .chip.bad { background:#fdecef; color:var(--dp-bad); border-color:#f5c2c7; }
 .dp-wrap .chip.na { background:#eef1f4; color:var(--dp-dim); border-color:#dbe3ea; }
 .dp-wrap .toprow { display:grid; grid-template-columns:2fr 1fr; gap:12px; align-items:start; }
 .dp-wrap .diagrow { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin-top:12px; }
 ...
 .dp-wrap .cal .d.has { cursor:pointer; color:#fff; font-weight:600; border-color:transparent; }
 .dp-wrap .cal .d.has:hover { outline:2px solid #2a5a82; }
 .dp-wrap .cal .d.cur { outline:2.5px solid #1a2530; }
 .dp-wrap .mon { font-weight:600; font-size:12px; margin:2px 0 6px; }
 .dp-wrap .calnav { display:flex; align-items:center; justify-content:space-between; margin:0 0 6px; }
 .dp-wrap .calnav button { font-size:13px; padding:0 8px; line-height:20px; }
 .dp-wrap .calnav .mon { margin:0; }
 .dp-wrap .legend { font-size:11px; color:var(--dp-dim); margin-top:9px; line-height:1.7; }
 .dp-wrap .sw { display:inline-block; width:10px; height:10px; border-radius:3px; margin-right:5px;
       vertical-align:-1px; }
 .dp-wrap .dp-verdict { background:#fff; border:1px solid var(--dp-line); border-radius:10px;
        padding:12px 16px; margin:0 0 12px; width:100%; max-width:none; box-sizing:border-box;
        display:block; }
 .dp-wrap .dp-verdict:empty { display:none; }
 .dp-wrap .dp-verdict.bad { border-left:5px solid var(--dp-bad); background:#fdecef; }
 .dp-wrap .dp-verdict.ok { border-left:5px solid var(--dp-ok); }
 .dp-wrap .dp-verdict b { font-size:14px; }
 .dp-wrap .dp-verdict .det { color:var(--dp-dim); font-size:12.5px; margin-top:4px; }
 .dp-wrap pre.summary { font:12px/1.45 ui-monospace,Consolas,monospace; margin:0; white-space:pre-wrap; }
 ...
 .dp-wrap .ctrls { display:flex; gap:8px; align-items:center; flex-wrap:wrap; margin-bottom:6px;
          min-height:30px; }
 .dp-wrap .ctrls label { font-size:11.5px; color:var(--dp-dim); }
 .dp-wrap .note { font-size:11.5px; color:var(--dp-dim); }
 .dp-wrap .notebar { margin:4px 2px 2px; border-top:1px solid #eef2f6; padding-top:6px; }
 .dp-wrap .dp-title { font-size:13px; margin:0 10px 0 0; text-transform:uppercase; letter-spacing:.04em;
       color:var(--dp-dim); }
 .dp-wrap .hd { font-size:12px; text-transform:uppercase; letter-spacing:.04em; color:var(--dp-dim);
       margin-right:4px; }
 .dp-wrap .axlbl { display:inline-flex; align-items:center; gap:5px; font-size:11px; color:var(--dp-dim); }
 .dp-wrap .empty { text-align:center; color:var(--dp-dim); padding:60px 10px; }
 .dp-wrap .whycard { margin-top:12px; }
 .dp-wrap .wtop { margin:0 0 8px; font-size:13px; }
 .dp-wrap .formula { font:12.5px/1.5 ui-monospace,Consolas,monospace; background:#f4f7fa;
            border:1px solid #e3e9ef; border-radius:7px; padding:8px 11px; margin:0 0 9px;
            overflow-x:auto; white-space:pre; }
 .dp-wrap .wtab { border-collapse:collapse; width:100%; font-size:12.5px; }
 .dp-wrap .wtab th { text-align:left; font-size:11px; text-transform:uppercase; letter-spacing:.03em;
            color:var(--dp-dim); border-bottom:1px solid var(--dp-line); padding:3px 8px; }
 .dp-wrap .wtab td { padding:3px 8px; border-bottom:1px solid #eef2f6; }
 .dp-wrap .wtab td:first-child { font-family:ui-monospace,Consolas,monospace; color:#2a5a82; }
 .dp-wrap .wtab .num { text-align:right; font-variant-numeric:tabular-nums; }
 .dp-wrap .wtab .wy { color:var(--dp-dim); font-size:11.5px; }
 .dp-wrap .wtab tr.drive { background:#fff8e6; }
 .dp-wrap .wtab tr.tot td { border-bottom:0; border-top:1px solid var(--dp-line); }
 .dp-wrap .gates { margin-top:9px; display:flex; flex-wrap:wrap; gap:6px; }
 .dp-wrap .gate { font-size:11px; background:#eef4fb; border:1px solid #d6e4f2; border-radius:11px;
         padding:2px 9px; color:#0b3d61; }
 .dp-wrap .gate .gv { color:#2a5a82; font-variant-numeric:tabular-nums; }
 @media (max-width:1100px) { .dp-wrap .toprow { grid-template-columns:1fr; }
   .dp-wrap .layout { grid-template-columns:1fr; } .dp-wrap .rail { position:static; }
   .dp-wrap .diagrow { grid-template-columns:1fr 1fr; } }
"""

#: Chrome for the STANDALONE mockup page only (page background, banner, title). The
#: embedded fragment above is fully scoped under .dp-wrap -- the audit found its bare
#: h2/body/:root rules restyling every heading and hijacking the host CSS tokens.
STANDALONE_CSS = r"""
 body { font:14px/1.5 -apple-system,"Segoe UI",Roboto,Arial,sans-serif; margin:0; color:var(--dp-ink);
        background:#f7f9fb; }
 ...
 .sub { color:var(--dp-dim); font-size:13px; }"""
...
document.addEventListener('keydown', e => {
  ...
  // handler serves the standalone mockup only. The old guard sniffed for '.diag-data', which the
  // classification section carries even when no calibration viewer exists -- an explicit handshake
  // (set by the station template before this script) cannot be defeated by page composition.
  if (window.__panelEmbedded) return;
...
<style>""" + PANEL_CSS + STANDALONE_CSS + """</style></head><body>
""" + PANEL_HDR + """
