...
for k, (inst, (ident, CL, wins)) in enumerate(INST.items()):
    ...
    axh.hist(med[np.isfinite(med)], bins=np.linspace(-XLIM, XLIM, 120), color="#6aa0d8")
    axh.axvline(0, color="0.5", lw=0.8)
    axh.axvline(np.nanmedian(meds[(rng >= 3000) & (rng <= 5000)]), color="#d62728", lw=1.6,
                label=f"3-5 km median = {np.nanmedian(meds[(rng>=3000)&(rng<=5000)]):+.2e}")
    axh.set_xlim(-XLIM, XLIM); axh.set_xlabel(r"median($\beta/r^2$)  [Mm$^{-1}$sr$^{-1}$ m$^{-2}$]")
    axh.set_ylabel("Count of range gates"); axh.legend(fontsize=8, loc="upper left")
